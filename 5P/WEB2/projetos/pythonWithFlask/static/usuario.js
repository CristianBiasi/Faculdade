...
function alterar(id) {
  fetch("http://127.0.0.1:5000/imovel/" + id)
    .then(resp => resp.json())
    .then(dados => {
      idusuarioatual = id;  // Corrigir aqui, estava idimovelatual
...
function salvar() {
  ...
  let url, metodo;
  if (idusuarioatual > 0) {
    url = "http://127.0.0.1:5000/imovel/" + idusuarioatual; // Enviar ID para PUT
    metodo = "PUT";
  } else {
    url = "http://127.0.0.1:5000/imovel";  // Para POST (novo cadastro)
    metodo = "POST";
  }
...
function listar() {
  ...
  fetch("http://127.0.0.1:5000/imovel")
    .then(resp => resp.json())
